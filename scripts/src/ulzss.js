  ...
  search: function () {
    ...
    if (d = this.hash[key]) {
      for (var i = 0; i < d.length; i++) {
        ...
        var j = 0;
        var b = 0;
        while (this.buffer.charCodeAt(real_pos + j) == this.buffer
          .charCodeAt(this.current + j) && j < ULZSS.MAX_LEN) {
...
ULZSS.encode = function (input) {
  ...
  while (window.next()) {
    if (window.flag) {
      flag |= mask;
      var code = window.match_pos + (window.match_len - ULZSS.MIN_LEN - 1) *
        4096;
